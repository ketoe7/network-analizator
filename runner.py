...
def main(nano_machines_amounts, iteration_number, router_position, vein, collision, mode, verbose, const_speed=False):
    ...
    for machines_amount in nano_machines_amounts:
        ...
        if vein.name == 'Short Vein':
            ...
            abstract_length = Vector.speed_for_d(vein.velocity, vein.radius, vein.radius - transmission_radius_range) * simulation_time


            machines_in_vein_amount = int(machines_amount * (math.pi * vein.radius**2 * vein.length)/blood_volume)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-V', '--vein', dest='vein', choices=['cephalic', 'short', 'custom'], action='store', default='custom', help='Vein for simulation ("cephalic" and "short" available)')
    ...
    elif args.vein == 'custom':
        pass
    else:
        raise NameError('Only "cephalic" and "short" veins are available!')
    # print(args.iterations)
    print(f'args.collision: {args.collision}')
    main(args.numbers, args.iterations, args.position, vein, args.collision, args.mode, args.verbose, args.const_speed)
